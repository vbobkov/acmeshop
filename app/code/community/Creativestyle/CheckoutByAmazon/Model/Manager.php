    ...
    protected function _getOrderByAmazonId($amazonId) {
        ...
        if ($paymentCollection->count()) {
            ...
            if ($payment->getParentId()) {
                ...
                Mage::helper('checkoutbyamazon')->throwException('No order with such Amazon Order ID');
            }
        } else {
            Mage::helper('checkoutbyamazon')->throwException('No order with such Amazon Order ID');
        }
            ...
            foreach ($doc->Message as $message) {
                if (isset($message->OrderReport->AmazonOrderID)) {
                    ...
                    if ($order->getId()) {
                        ...
                        Mage::helper('checkoutbyamazon')->throwException('Order does not exist', null, array('area' => 'Amazon MWS Reports'));
                    }
                }
            }
    ...
    public function newOrder($amazonOrder) {
        if ($amazonOrder->issetAmazonOrderID()) {
            ...
            if ($order->getId()) {
                ...
                Mage::helper('checkoutbyamazon')->throwException('Order does not exist');
            }
        } else {
    ...
    public function cancelOrder($amazonOrder, $sender = 'IOPN') {
        if ($amazonOrder->issetAmazonOrderID() || $amazonOrder->issetAmazonOrderId()) {
            ...
            if ($order->getId()) {
                ...
                Mage::helper('checkoutbyamazon')->throwException('Order does not exist', null, array('area' => ($sender == 'Amazon MWS' ? 'Amazon MWS Orders' : 'Amazon IOPN')));
            }
        }
    }
