...
class Creativestyle_CheckoutByAmazon_Model_Observer extends Creativestyle_CheckoutByAmazon_Model_Abstract {
    ...
    public function setCartSecure($observer) {
        try {
            if (Mage::getStoreConfig(Creativestyle_CheckoutByAmazon_Block_Abstract::XML_PATH_OVERLAY_ENABLED)) {
                Mage::getConfig()->getNode('frontend/secure_url')->addChild('checkout_cart', '/checkout/cart');
            }
        } catch (Exception $e) {
            if (!($e instanceof Creativestyle_CheckoutByAmazon_Exception)) {
                Creativestyle_CheckoutByAmazon_Model_Logger::logException(
    ...
    public function logApiCall($observer) {
        ...
        Creativestyle_CheckoutByAmazon_Model_Logger::logApiCall($host, $action, null, $requestMethod, $headers, $get, $post, $file, $responseCode, $response);
        return $this;
    }
