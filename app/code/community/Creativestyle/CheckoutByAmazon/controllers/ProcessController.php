...
class Creativestyle_CheckoutByAmazon_ProcessController extends Mage_Core_Controller_Front_Action {
    ...
    protected function _getOrderId($amazonOrderId) {
        $paymentCollection = Mage::getModel('sales/order_payment')->getCollection()
                ->addAttributeToFilter('last_trans_id', $amazonOrderId)
                ->load();
    ...
    public function iopnAction() {
        if (Mage::helper('checkoutbyamazon')->getConfigData('iopn_active')) {
            ...
            try {
                ...
                switch ($notificationType) {
                    case 'NewOrderNotification':
                        ...
                        if (isset($result['code']) && ($result['code'] == 200)) {
                            ...
                            $amazonOrderId = $order->issetAmazonOrderID() ? $order->getAmazonOrderID() : null;
                            Creativestyle_CheckoutByAmazon_Model_Logger::logIopn(
                                $notificationType,
                                isset($params['UUID']) ? $params['UUID'] : '',
                                $amazonOrderId ? $this->_getOrderId($amazonOrderId) : null,
                ...
                $amazonOrderId = null;
                if (isset($result['order']) && is_object($result['order'])) {
                    $order = $result['order'];
                    $amazonOrderId = $order->issetAmazonOrderID() ? $order->getAmazonOrderID() : null;
                }
                Creativestyle_CheckoutByAmazon_Model_Logger::logIopn(
                ...
                if (!($e instanceof Creativestyle_CheckoutByAmazon_Exception)) {
                    Creativestyle_CheckoutByAmazon_Model_Logger::logException(
                        ...
                        'Amazon IOPN',
                        isset($params['UUID']) ? $params['UUID'] : '',
                        isset($result['order']) ? $this->_getOrderId($result['order']) : null
                    );
                }
