...
class Creativestyle_CheckoutByAmazon_Helper_Debug extends Mage_Core_Helper_Abstract {
    ...
    protected function _getEventObservers($event) {
        $result = array();
        $observers = Mage::getConfig()->getNode('global/events/' . $event . '/observers');
        if ($observers) {
            $observers = $observers->asArray();
            foreach ($observers as $observer) {
                $function = null;
                if (isset($observer['class']) && $observer['class']) {
                    $function .= Mage::getConfig()->getModelClassName($observer['class']);
                }
                if (isset($observer['method']) && $observer['method']) {
                    if ($function) $function .= '::';
                    $function .= $observer['method'] . '()';
                }
    ...
    public function getDebugInfo($area = null) {
        if (null === $this->_debugInfo) {
            ...
            $general = array(
                ...
                'CreativeCore extension version' => (string)Mage::getConfig()->getNode('modules/Creativestyle_Core/version'),
            ...
            $amazonAppearance['Show in overlay'][0] = Mage::getStoreConfig(Creativestyle_CheckoutByAmazon_Block_Abstract::XML_PATH_OVERLAY_ENABLED) ? 'Yes' : 'No';
            $amazonAppearance['Overlay width'][0] = Mage::getStoreConfig(Creativestyle_CheckoutByAmazon_Block_Abstract::XML_PATH_OVERLAY_WIDTH);
            ...
            if ($storeCollection->count()) {
                foreach ($storeCollection as $store) {
                    ...
                    $amazonAppearance['Show in overlay'][$store->getCode()] = Mage::getStoreConfig(Creativestyle_CheckoutByAmazon_Block_Abstract::XML_PATH_OVERLAY_ENABLED, $store->getId()) ? 'Yes' : 'No';
                    $amazonAppearance['Overlay width'][$store->getCode()] = Mage::getStoreConfig(Creativestyle_CheckoutByAmazon_Block_Abstract::XML_PATH_OVERLAY_WIDTH, $store->getId());
