	...
	if (!empty($topic))
	{
		$request = $smcFunc['db_query']('', '
			SELECT
				...
				t.id_first_msg, mf.subject, mf.hiddenOption, mf.hiddenValue, mf.hiddenInfo,
		...
		$context['can_hide_post'] = (allowedTo('hide_post_any') ||  ($user_info['id'] == $ID_MEMBER_POSTER && allowedTo('hide_post_own'))) && !empty($modSettings['allow_hiddenPost']);
...
function Post2()
{
	...
	$msgOptions = array(
		...
		'hiddenInfo' => (empty($_POST['hiddenInfo']) ? '' : $_POST['hiddenInfo']),
