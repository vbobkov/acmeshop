...
function template_main()
{
	...
	echo '<input id="hiddenOption-hide_moderator" name="hiddenOption-hide_moderator" type="checkbox"><label for="hiddenOption-hide_moderator">Sensitive information? Make this post private (visible only to you and our staff).</label>';
	...
	echo '
			jQuery(document).ready(function() {
				jQuery("#hiddenOption-hide_moderator").change(function(event) {
					if(jQuery(this).prop("checked")) {
						jQuery("#hiddenOption").val(5);
					}
					else {
						jQuery("#hiddenOption").val(0);
					}
				});
			});
	...
	if (isset($context['previous_posts']) && count($context['previous_posts']) > 0)
	{
		...
		foreach ($context['previous_posts'] as $post)
		{
			...
			if ($context['can_quote'])
			{
				echo '<ul class="reset smalltext quickbuttons" id="msg_', $post['id'], '_quote">';
				if ($post['can_view_post']) {
					echo '<li class="quote_button"><a href="#postmodify" onclick="return insertQuoteFast(', $post['id'], ');"><span>',$txt['bbc_quote'],'</span></a></li>';
				}
				echo '</ul>';
			}
